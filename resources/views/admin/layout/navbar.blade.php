        ...
        if (query.length > 1) { // Mulai pencarian setelah 2 karakter
            axios.get(`{{ route('search') }}?query=${query}`)
                .then(response => {
                    ...
                    appendResults("Pegawai", data.pegawais, 'nama_pegawai', 'admin/pegawai');
                    appendResults("Users", data.users, 'nama_user', 'admin/user');
                    appendResults("Jabatan", data.jabatans, 'nama_jabatan', 'admin/jabatan');
                    appendResults("Jurusan", data.jurusans, 'nama_jurusan', 'admin/jurusan');
                    appendResults("Kelas", data.kelas, 'nama_kelas', 'admin/kelas');
                    appendResults("Prestasi", data.prestasis, 'nama_prestasi', 'admin/prestasi');
                    appendResults("Tugas Tambahan", data.tugastambahans, 'nama_tugas', 'admin/laporan/tugas-tambahan');
                    appendResults("Mata Pelajaran", data.mapels, 'nama_pelajaran', 'admin/mapel');
                    ...
                    if (
                        ...
                        (!data.jurusans || data.jurusans.length === 0) &&
